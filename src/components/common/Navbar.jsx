        ...
        <div className='container'>
          ...
          <div className='navbar-main-bottom flex align-center justify-between'>
              ...
              <ul className={`category-list ${showCategory ? 'show-category-list' : ''}`} onClick={toggleCategory}>
                {
                  ...
                  )) : <li className='category-item text-uppercase text-dark fs-12'>No Categories</li>
                }
              </ul>
            </div>
            ...
            <ul className="navbar-nav flex align-center">
              {
                categories && categories.length > 0 ? categories.slice(0, 6).map((category, idx) => (
                  <li className='nav-item' key={idx}>
                    <Link to={`category/${category.slug}`} className='nav-link no-wrap'>
                      {category.name}
                    </Link>
                  </li>
                )) : <li className='nav-item text-uppercase text-dark fs-12'>No Categories</li>
              }
            </ul>
          </div>
        </div>
